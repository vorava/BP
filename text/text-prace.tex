...
Celkově byly natrénovány 4 modely. Oba modely popsané v sekci \ref{sekce:detektor_obliceju_implementace} s oběma datasety ze sekce \ref{sekce:priprava_dat}. Jednotlivé parametry trénovaných modelů zachycuje tabulka.
...
Nástroje pro porovnávání modelů a experimenty s nimi jsou potřeba proto, aby bylo možné zhodnotit rozdíly mezi akcelerovaným a neakcelerovaným řešením a porovnat implementované detektory s existujícími. Tato část popisuje skripty pro zpracování výsledků detektorů obličejů a způsoby jejich porovnávání pro možnosti experimentů v další kapitole. Je zde vylíčeno jak se každá měřená metrika vyhodnocuje a jak vypadají výstupy experimentačních programů.

\subsection*{Data pro experimenty}
Mezi data, která jsou hodnocena za účelem srovnání detekčních sítí, nepatří jen data získaná z GUI aplikace, ale také evaluace modelů na validačních datasetech. 
